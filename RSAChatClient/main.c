...
int main(int argc, const char *argv[]) {
	...
	inet_pton(AF_INET, argv[2], &serverAddy.sin_addr.s_addr);// use address string from arg2 in pton to make the ipv4 address
	...
	if (argc == 3)	{
		...
		if ( ( sockfd = socket(AF_INET,SOCK_STREAM,IPPROTO_TCP) ) > -1 ) {//create generic sockaddr structure from the sockaddr_in at the address we made
			if ( connect(sockfd, (struct sockaddr*)&serverAddy, INET_ADDRSTRLEN)) {
